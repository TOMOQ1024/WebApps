...
(() => {
  ...
  const rootPath = getFileRef("Minesweeper", "root");
  let width = 10;
  let height = 10;
  let mines = 15;
  let flag = 0;
  ...
  let tiles;
  const img = new Image();
  img.src = rootPath + "assets/Minesweeper.png";

  let len = 64;
  let cvs = document.createElement("canvas");
  cvs.className = "cvs-single";
  cvs.style.backgroundImage = `url(${rootPath}assets/808080-c.png)`;
  cvs.style.backgroundSize = "10% 10%";
  cvs.width = len * width;
  ...
  let flagcvs = document.createElement("canvas");
  flagcvs.id = "flagcanvas";
  flagcvs.style.position = "absolute";
  flagcvs.style.display = "none";
  flagcvs.style.top = "0";
  flagcvs.style.left = "50%";
  flagcvs.style.zIndex = "100";
  let flagctx = flagcvs.getContext("2d");
  document.getElementById("main").appendChild(cvs);
  document.getElementById("main").appendChild(flagcvs);
